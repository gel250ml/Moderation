...
@router.post(
    ...
    response_model=ApproveProductResponse,
    responses={
        401: {"model": ErrorResponse, "description": "Unauthorized"},
        403: {"model": ErrorResponse, "description": "Card assigned to another moderator"},
        404: {"model": ErrorResponse, "description": "Product not found in moderation queue"},
        409: {"model": ErrorResponse, "description": "Wrong card status or product has no SKUs"},
        500: {"model": ErrorResponse, "description": "B2B event delivery failed"},
    },
)
async def approve_product(
    product_id: UUID,
    data: ApproveProductRequest | None = None,
    moderator_id: UUID = Depends(get_current_moderator_id),
    db: AsyncSession = Depends(get_db),
) -> ApproveProductResponse:
    service = ApproveService(db)
    return await service.approve_by_product_id(
        product_id=product_id,
        moderator_id=moderator_id,
        comment=data.moderator_comment if data is not None else None,
    )
